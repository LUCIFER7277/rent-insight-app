...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const themeClass = cn({
    ...
    "bg-[#F0F4FF] text-slate-900 min-h-screen": isManager || isOwnerPortal || isAdminPortal,
    "bg-[#FBFBFC] text-slate-900 min-h-screen": isEarner || (!persona && !isOwnerPortal && !isAdminPortal),
  ...
  if (!persona && !isOwnerAuthenticated && !isAdminPortal) {
    return (
      <div className="min-h-[100dvh] bg-[#FBFBFC] w-full">
        {((location.startsWith("/manager") || isAdminPortal) && !isOwnerAuthenticated) && <ActiveOwnerBar />}
        {children}
      </div>
    );
  }
  ...
  const navItems = isOwnerPortal ? [
    ...
    { href: "/manager/owners", icon: User, label: "Owners" },
  ...
  const extraNavBase = isOwnerPortal ? [] : [
  ...
  return (
    <div className={cn("min-h-[100dvh] w-full flex flex-col pb-16 md:pb-0 md:flex-row", themeClass)}>
        ...
        <div className="p-6">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest leading-none">{isOwnerPortal ? "Owner Portal" : isAdminPortal ? "Admin Portal" : "Homes"}</p>
            </div>
          </div>
          <div className="mt-3 flex items-center gap-2">
            <span className="text-xs font-bold px-2 py-0.5 bg-primary/10 text-primary rounded-full">{isOwnerPortal ? "PG OWNER" : isAdminPortal ? "ADMIN" : persona}</span>
          </div>
        </div>
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto overflow-y-auto">
        ...
        <div className={cn(
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div>
              ...
              <span className={cn("text-[9px] font-bold ml-1 opacity-60 uppercase tracking-widest", isGuard || isBroker ? "text-white" : "text-foreground")}>{isOwnerPortal ? "Owner Portal" : "Homes"}</span>
            </div>
          </div>
        ...
        {((isManager || location.startsWith("/manager") || isAdminPortal) && !isOwnerPortal) && <ActiveOwnerBar />}
        {children}
      </main>
