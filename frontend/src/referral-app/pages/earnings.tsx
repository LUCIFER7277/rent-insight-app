    ...
    <Layout>
      ...
      <div className="p-4 md:p-6 max-w-3xl mx-auto space-y-6">
        ...
        <div className="bg-gradient-to-br from-green-500 to-emerald-600 text-white rounded-3xl p-6 shadow-xl">
          ...
          <p className="text-6xl font-black tracking-tight">â‚¹{dashboard.referrer.totalEarned.toLocaleString()}</p>
          <div className="flex gap-6 mt-5 pt-4 border-t border-white/20">
            <div>
              <p className="text-white/60 text-xs mb-1">Paid Out</p>
              <p className="text-xl font-black text-green-200">â‚¹{dashboard.referrer.paidEarnings.toLocaleString()}</p>
            </div>
            <div>
              <p className="text-white/60 text-xs mb-1">Pending</p>
              <p className="text-xl font-black text-yellow-200">â‚¹{dashboard.referrer.pendingEarnings.toLocaleString()}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-card border border-border rounded-2xl p-4">
            ...
            <p className="text-2xl font-black text-foreground">â‚¹{(dashboard.referrer.verifiedReferrals * 50).toLocaleString()}</p>
            <p className="text-xs text-muted-foreground mt-1">{dashboard.referrer.verifiedReferrals} leads Ã— â‚¹50</p>
          </div>
          <div className="bg-card border border-border rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-2xl font-black text-foreground">â‚¹{(dashboard.referrer.bookedReferrals * 500).toLocaleString()}</p>
            <p className="text-xs text-muted-foreground mt-1">{dashboard.referrer.bookedReferrals} bookings Ã— â‚¹500</p>
          </div>
          <div className="bg-card border border-border rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-2xl font-black text-orange-600">â‚¹{dashboard.referrer.pendingEarnings.toLocaleString()}</p>
          ...
          <div className="bg-card border border-border rounded-2xl p-4">
            ...
            <p className="text-xs text-muted-foreground mt-1">Leads â†’ bookings</p>
          </div>
        </div>
        ...
        {chart && chart.length > 0 && (
          <div className="bg-card border border-border rounded-2xl p-5">
            <div className="flex items-center justify-between mb-5">
              ...
              <span className="text-sm font-bold text-primary">â‚¹{totalFromChart.toLocaleString()} total</span>
            </div>
              ...
              {chart.map((d: any, i: number) => {
                ...
                return (
                  <div key={d.month} className="flex-1 flex flex-col items-center gap-1">
                    <p className="text-[10px] font-bold text-primary">â‚¹{(d.earned / 1000).toFixed(1)}k</p>
        ...
        <div className="bg-blue-50 border border-blue-100 rounded-2xl p-5">
          <h3 className="font-bold text-blue-800 mb-3">ðŸ’³ Payout Schedule</h3>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-blue-700">Verification bonus (â‚¹50)</span>
              <span className="font-bold text-blue-800">Within 48h of verification</span>
            </div>
            <div className="flex justify-between">
              <span className="text-blue-700">Booking bonus (â‚¹500)</span>
              <span className="font-bold text-blue-800">Within 48h of move-in</span>
            </div>
            <div className="flex justify-between">
              <span className="text-blue-700">Minimum payout</span>
              <span className="font-bold text-blue-800">â‚¹100</span>
            </div>
            <div className="flex justify-between">
              <span className="text-blue-700">Processing days</span>
              <span className="font-bold text-blue-800">Monday â€“ Friday</span>
            </div>
          </div>
