...
import {
  Bell, BellOff, CheckCheck, Home, Video, TrendingUp, TrendingDown,
  Lock, Package, AlertTriangle, Flame, DollarSign, Users, RefreshCw
} from "lucide-react";
...
export default function NotificationsPage() {
  ...
  const { ownerToken, ownerUser, isOwnerAuthenticated, setOwnerUnreadCount } = useOwnerStore();
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      if (isOwnerAuthenticated && ownerToken) {
        // Fetch owner-specific smart notifications
        ...
        if (res.ok) {
          ...
          setOwnerUnreadCount(data.filter((n: any) => !n.isRead).length);
        } else {
          setNotifications([]);
        }
      } else if (referrer) {
        // Non-owner: fall back to activity feed
        const res = await fetch(`${BASE}/api/activity/${referrer.id}`);
        ...
        setNotifications(data.map((item: any) => ({
          ...
          category: "default",
          title: item.label,
          message: item.xp ? `Earned ${item.xp} XP` : "Activity recorded",
          amount: item.amount,
  ...
  }, [isOwnerAuthenticated, ownerToken, referrer, refreshKey]);
  ...
  const handleMarkAll = async () => {
    ...
    setOwnerUnreadCount(0); // Immediately clear sidebar badge
  ...
  const handleMarkOne = async (id: string) => {
    ...
    setNotifications(prev => {
      ...
      setOwnerUnreadCount(updated.filter(n => !n.isRead).length); // Update badge
      return updated;
    });
  };
