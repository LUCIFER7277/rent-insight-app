...
import { Plus, Building2, MapPin, Edit, ToggleLeft, ToggleRight, Star, UserCheck } from "lucide-react";
...
export default function AdminPropertiesPage() {
  ...
  const { data: properties, isLoading, refetch } = useGetManagerProperties(referrer?.id ?? 0, activeOwnerId);
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h1 className="text-2xl font-black font-display text-slate-900">
              {activeOwner ? `${activeOwner.name}'s PGs` : "My Properties"}
            </h1>
            ...
            {properties.map((p, i) => (
              <motion.div key={p.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  ...
                  <div className="flex flex-col items-end gap-2">
                    {p.avgRating && (
                      <div className="flex items-center gap-1 bg-yellow-50 border border-yellow-100 rounded-lg px-2 py-1">
                        ...
                        <span className="text-xs font-bold">{(p.avgRating as number).toFixed(1)}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setLocation(`/admin/properties/${p.id}/rooms`)}
