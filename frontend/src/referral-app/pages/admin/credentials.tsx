...
import { useGetAdminOwners, useRotateAdminOwnerPassword } from "@/referral-app/api";
...
export default function AdminCredentialsPage() {
  ...
  const { data: owners = [], isLoading } = useGetAdminOwners();
  const { mutate: rotatePassword, isPending: isRotating } = useRotateAdminOwnerPassword();
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-slate-500 text-sm">
              Single source of truth for all {isLoading ? "..." : owners.length} owner accounts
            </p>
          </div>
            ...
            <TableBody>
              {filtered.map((o) => (
                <TableRow key={o.id}>
                  <TableCell>
                    <button
                      onClick={() => setLocation(`/admin/owners/${o.id}`)}
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1">
                      ...
                      <button onClick={() => copy(o.username, "Username")} className="p-1 hover:bg-slate-100 rounded">
