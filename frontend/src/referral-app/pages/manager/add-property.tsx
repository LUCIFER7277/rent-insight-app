...
export default function ManagerAddPropertyPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (!ownerToken) {
      ...
      try {
        await createPropertyMock.mutateAsync({
          data: {
  ...
  return (
    <Layout>
      <div className="p-4 md:p-6 space-y-6 max-w-2xl mx-auto">
        <button onClick={() => setLocation("/manager/properties")} className="flex items-center gap-2 text-slate-500 hover:text-slate-800 text-sm font-medium">
