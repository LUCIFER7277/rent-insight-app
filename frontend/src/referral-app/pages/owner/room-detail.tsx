...
import { useGetRealOwnerRooms, useUpdateRealOwnerRoomStatus } from "@/referral-app/api";
...
export default function OwnerRoomDetailPage() {
  const { id: pid, roomId } = useParams() || {};
  const [, setLocation] = useLocation();
  const { toast } = useToast();

  const { ownerToken } = useOwnerStore();
  const { data: realRoomsData, isLoading: isRoomsLoading, refetch } = useGetRealOwnerRooms(ownerToken || "");
  const updateStatusMut = useUpdateRealOwnerRoomStatus();

  const room = useMemo(() => {
    if (!realRoomsData || !roomId) return null;
    const { rooms: br, roomStatuses: bs, property } = realRoomsData;
    ...
    return {
      ...
      beds: x.bedsTotal || 2,
      status: s.kind || "quoted", // Map "vacant" to "quoted" for UI parity if needed, but let's stick to s.kind
    ...
    return (
      <Layout>
        <div className="p-6 max-w-2xl mx-auto">
          <button onClick={() => setLocation(`/owner/properties/${pid}/rooms`)} className="flex items-center gap-2 text-slate-500 text-sm mb-4">
  ...
  const handleUpdate = async (type: 'readiness' | 'usp') => {
    ...
    try {
      ...
      await updateStatusMut.mutateAsync({ token: ownerToken, roomId: room.id, data: payload });
  ...
  const shareRoom = () => {
    const pname = room.property?.name || "the PG";
    const area = room.property?.area || "";
    const text = `Hi! Room *${room.roomNumber}* at *${pname}*${area ? ` (${area})` : ""} — ${room.beds} bed, ${room.commercial || "quoted"}. Rent ₹${room.expectedRent?.toLocaleString()}/mo. Interested? Let me know!`;
  ...
  const mapsLink = () => {
    const q = room.property?.address || room.property?.area || "Bangalore";
    window.open(`https://maps.google.com/?q=${encodeURIComponent(q)}`, "_blank");
  };

  const scheduleVisit = () => toast({ title: "Schedule Visit", description: "Visit scheduling modal will open here." });
  const createBooking = () => toast({ title: "Create Booking", description: "Booking flow will start here." });
        ...
        <div className="p-4 md:p-6 space-y-6 max-w-5xl mx-auto">
          ...
          <div className="flex items-start justify-between">
            <div>
              <p className="text-xs font-bold tracking-widest text-orange-700/80 uppercase mb-1">
                ROOM · {room.property?.name || "Gharpayy"} · Floor {room.floorNumber}
              </p>
              <h1 className="text-3xl font-display font-black text-slate-900 tracking-tight">
                Room #{room.roomNumber}
              </h1>
            </div>
            <Button variant="outline" className="rounded-full bg-white font-medium" onClick={() => setLocation(`/owner/properties/${pid}/rooms`)}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-[1fr_320px] gap-6">
            ...
            <div className="space-y-6">
              ...
              {/* Readiness Card */}
              <div className="bg-white border border-amber-200 rounded-2xl p-5 shadow-sm relative">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-5">
                  <div className="space-y-1.5">
                    ...
                    <select className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500"
                      value={draft.commercial} onChange={(e) => setDraft({...draft, commercial: e.target.value})}>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500"
                      value={draft.operational} onChange={(e) => setDraft({...draft, operational: e.target.value})}>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500"
                      value={draft.turnaround} onChange={(e) => setDraft({...draft, turnaround: e.target.value})}>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <input type="date" className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500"
                        value={draft.availableFrom} onChange={(e) => setDraft({...draft, availableFrom: e.target.value})} />
                    </div>
                  </div>
                  <div className="flex items-end">
                    <Button className="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-full shadow-sm" onClick={() => handleUpdate('readiness')}>
              ...
              <div className="bg-white border border-amber-200 rounded-2xl p-5 shadow-sm">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="font-display font-medium text-xl text-slate-900">
                    USP & selling points
                  </h3>
                  <Button className="bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-full px-5 shadow-sm" onClick={() => handleUpdate('usp')}>
                    Save USP
                  </Button>
                </div>
