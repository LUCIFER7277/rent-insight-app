...
import { useGetRealOwnerProperties, useGetOwnerStats } from "@/referral-app/api";
...
export default function OwnerInventoryPage() {
  ...
  const { data: stats, isLoading: isStatsLoading, isError: isStatsError } = useGetOwnerStats(ownerToken);
  ...
  const statsByPropertyId = useMemo(
    () => new Map<string, any>((stats?.properties ?? []).map((p) => [p.propertyId, p])),
    [stats]
  );
