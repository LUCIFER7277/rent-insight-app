...
import type { Lead } from "../../contracts/entities.js";
import type { Tour } from "../../contracts/entities.js";
...
export function registerStatsRoutes(app: FastifyInstance) {
  ...
  app.get("/api/stats/admin", async (req, reply) => {
    const tenantId = "gharpayy"; // Hardcoded for admin dashboard
    const { ownerId } = req.query as { ownerId?: string };
    
    // Aggregation for properties
    const matchQuery: any = { tenantId };
    if (ownerId) matchQuery.ownerId = ownerId;

    const properties = await col<any>("properties").find(matchQuery).toArray();
    const propIds = properties.map(p => p.customId || p._id);
    const rooms = await col<any>("rooms").find({ propertyId: { $in: propIds } }).toArray();
    const roomIds = rooms.map(r => r.customId || r._id);
    const roomStatuses = await col<any>("room_statuses").find({ roomId: { $in: roomIds } }).toArray();
    ...
    rooms.forEach(room => {
      const status = roomStatuses.find(s => s.roomId === (room.customId || room._id));
      const kind = status?.kind || 'vacant';
      const rent = status?.actualRent || 0;
      
      if (kind === 'vacant' || kind === 'vacating') {
        vacantRooms++;
      } else {
        monthlyRevenue += rent;
      }
    ...
    if (properties.length > 0) {
      const totalFoodRating = properties.reduce((sum, p) => sum + (p.foodRating || 0), 0);
      avgRating = totalFoodRating / properties.length;
    }

    const occupiedRooms = Math.max(0, totalRooms - vacantRooms);
    const occupancyRate = totalRooms > 0 ? occupiedRooms / totalRooms : 0;
    
    // Aggregation for leads
    const leadsQuery: any = { tenantId };
    // We don't have ownerId on leads currently, but if we did, we would filter here.
    const totalLeadsReceived = ownerId ? 0 : await col("leads").countDocuments(leadsQuery);
    
    // Aggregation for bookings (leads won or completed tours, let's use tours for now)
    const toursQuery: any = { tenantId, status: "completed" };
    const totalBookings = ownerId ? 0 : await col("tours").countDocuments(toursQuery);
    const conversionRate = totalLeadsReceived > 0 ? Math.round((totalBookings / totalLeadsReceived) * 100) : 0;
    
    // Other metrics can be derived or default
    const overflowLeadsThisMonth = 0;
  ...
  app.get("/api/admin/notifications", async (req, reply) => {
    const list = await col("notifications")
      .find({ tenantId: "gharpayy", recipient: "ADMIN" })
      .sort({ createdAt: -1 })
      .limit(50)
      .toArray();
    return reply.send(list.map(n => ({
      id: n._id,
      title: n.title,
      message: n.message,
      type: n.type,
  ...
  app.post("/api/admin/notifications/mark-read", async (req, reply) => {
    await col("notifications").updateMany(
      { tenantId: "gharpayy", recipient: "ADMIN", isRead: false },
      { $set: { isRead: true } }
    );
